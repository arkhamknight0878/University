...
int string_table::hash_function(int key_) { return (key_ % main_scope_size + 1); }

void string_table::resize()
{
...
string_table::~string_table() { delete[] hash_table; }
...
int string_table::add(int key_, int ind)
{
	int hash_index = hash_function(key_);
...
void string_table::erase(int key_)
{
	int hash_index = hash_function(key_);
...
ostream& operator<<(ostream& out_, const string_table& hash_)
{
	cout << left
		<< setw(8) << "Index"
		<< setw(12) << "Index_TL"
